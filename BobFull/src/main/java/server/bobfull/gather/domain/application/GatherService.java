...
public class GatherService {
    ...
    public Boolean proposalJoin(Long memberId, Long gatherId) {
        Member friend = memberService.findByMemberId(memberId);
    ...
    @Transactional
    public Boolean proposalConfirm(Long memberId,Boolean confirm,
                                Long confirmMemberId,
                                Long friendId) {
    ...
    public List<GatherResponseDto> getAllGathersForList(Long memberId,String bigLocation,String smallLocation) {
        ...
        List<Gather> filteredGathers = gathers.stream()
            .filter(gather -> gather.getBigLocation()
                .equals(bigLocation))
            .collect(Collectors.toList());

        List<Gather> gathersResult = filteredGathers.stream()
            .filter(gather -> gather.getSmallLocation()
                .equals(smallLocation))
            .collect(Collectors.toList());
